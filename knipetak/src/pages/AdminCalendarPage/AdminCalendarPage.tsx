...
const AdminCalendarPage: React.FC = () => {
    ...
    const [historyViewType, setHistoryViewType] = useState<'month' | 'year'>('month');
    
    // Nye tilstander for redigering og kansellering
    ...
    const filterBookings = () => {
        ...
        return bookings.filter(booking => {
            ...
            // For aktive bookinger, filtrer bort tidligere behandlinger
            if (!showHistory) {
                const bookingEndTime = new Date(bookingDate);
                bookingEndTime.setMinutes(bookingEndTime.getMinutes() + booking.duration);
    ...
    const formatTimeRange = (startTime: Date, duration: number) => {
        ...
        const end = new Date(start.getTime() + duration * 60000); // Konverterer minutter til millisekunder
    ...
    // Nye funksjoner for å håndtere redigering og kansellering
    ...
    const handleEditSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setBookings(bookings.map(booking => 
                booking.bookingId === editingBooking.bookingId ? editingBooking : booking
            ));
            ...
            setErrorMessage('Det oppsto en feil ved oppdatering av booking. Vennligst prøv igjen.');
        } finally {
    ...
    const handleCancelSubmit = async () => {
        ...
        try {
            ...
            setErrorMessage('Det oppsto en feil ved kansellering av booking. Vennligst prøv igjen.');
        } finally {
            setIsSubmitting(false);
        }
    };
                ...
                {!showHistory ? (
                    ...
                    <div className="filter-section">
                        <div className="history-view-toggle">
                            <button 
                                className={`toggle-button ${historyViewType === 'month' ? 'active' : ''}`}
                                onClick={() => {
                            ...
                            <button 
                                className={`toggle-button ${historyViewType === 'year' ? 'active' : ''}`}
                                onClick={() => {
                    ...
                    <div className="bookings-list">
                        {filteredBookings.length > 0 ? (
                            filteredBookings.map((booking) => (
                                <div key={booking.bookingId} className="booking-item">
                                    <div className="booking-info">
                                        ...
                                        <p className="booking-duration">Varighet: {booking.duration} minutter</p>
                                        <p className={`booking-status status-${booking.status}`}>
                                            Status: {booking.status === 'pending' ? 'Venter' : 
                                        ...
                                        <div className="booking-actions">
                                            ...
                                            <button 
                                                ...
                                                onClick={() => booking.bookingId && handleCancelClick(booking.bookingId)}
                                                disabled={booking.status === 'cancelled' || !booking.bookingId}
                                            >
                                                Kanseller
                                            </button>
                                        </div>
                            ...
                            <div className="no-bookings">
                                <p>
                                    {showHistory 
                                        ? `Ingen tidligere behandlinger funnet for valgt ${historyViewType === 'year' ? 'år' : 'måned'}.`
                            ...
                            <form onSubmit={handleEditSubmit}>
                                <div className="form-group">
                                    ...
                                    <input
                                        ...
                                        id="date"
                                        name="date"
                                        value={editingBooking.date instanceof Date 
                                            ? editingBooking.date.toISOString().split('T')[0] 
                                            : new Date(editingBooking.date).toISOString().split('T')[0]}
                                ...
                                <div className="form-group">
                                    ...
                                    <input
                                        ...
                                        id="time"
                                        name="time"
                                        value={editingBooking.timeslot.start instanceof Date 
                                            ? editingBooking.timeslot.start.toTimeString().substring(0, 5) 
                                            : new Date(editingBooking.timeslot.start).toTimeString().substring(0, 5)}
                                        onChange={handleInputChange}
                                        required
                                    />
                                </div>
